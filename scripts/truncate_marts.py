#!/usr/bin/env python
"""
truncate_marts.py — Truncate all mart tables (except dim_organization which is static ref).

Idempotent cleanup script. Use before retrying transform or for data reloads.
...
Usage (in-pod via spark-submit):
    spark-submit scripts/truncate_marts.py
...
try:
    ...
    # Exclude dim_organization (static reference data, seeded at DDL time)
    ...
    print("\n[OK] Mart tables truncated (dim_organization untouched).")
